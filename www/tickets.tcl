...
# part of the Customer Service package 
...
set title "#customer-service.Customer_Serivce#"
...
set page_name [ns_urldecode $message]
set page_title [ns_urldecode $internal_notes]
...
if { $form_posted } {
    ...
    if { ( $next_mode eq "v" || $next_mode eq "l" ) && [string match -nocase GET $http_header_method] } {
        ...
        ns_log Notice "tickets.tcl(223): Setting redirect_before_v_p "
        set redirect_before_v_p 1
    } 
    if { $mode eq "d" } {
        if { $delete_p } {
            ns_log Notice "tickets.tcl validated for d"
            set validated_p 1
        } elseif { $read_p } {
            set mode "l"
            set next_mode ""
    ...
    ns_log Notice "tickets.tcl(157): mode $mode next_mode $next_mode"
    if { $mode eq "w" } {
        if { $write_p } {
            set validated_p 1
        } elseif { $read_p } {
    ...
    ns_log Notice "tickets.tcl(169): mode $mode next_mode $next_mode"
    if { $mode eq "r" || $mode eq "a" } {
        if { $write_p } {
            if { [qf_is_natural_number $page_template_id_from_url ] } {
                set validated_p 1
                ns_log Notice "tickets.tcl validated for $mode"
            } elseif { $read_p } {
    ...
    ns_log Notice "tickets.tcl(185): mode $mode next_mode $next_mode"
    if { $mode eq "t" } {
        if { ( $write_p || $user_id > 0 ) && ([qw_page_id_exists $page_id $instance_id] || [qw_page_id_exists $page_id_from_url $instance_id] ) } {
            # complete validation occurs while trashing.
            set validated_p 1
            ns_log Notice "tickets.tcl validated for t"
        } elseif { $read_p } {
            set mode "l"
        } else {
            set mode ""
    ...
    ns_log Notice "tickets.tcl(197): mode $mode next_mode $next_mode"
    if { $mode eq "e" } {
        ...
        if { $write_p || ( $create_p && !$template_exists_p ) } {
            ...
            if { $page_title eq "" && $page_template_id eq "" } {
                ...
                set page_title_length [parameter::get -instance_id $instance_id -parameter PageTitleLen -default 80]
            ...
            ns_log Notice "tickets.tcl(226): url $url"
            # page_name is pretty version of url, cannot be blank
            if { $page_name eq "" } {
                set page_name $url
            } else {
                set page_name_length [parameter::get -instance_id $instance_id -parameter PageNameLen -default 40]
    ...
    ns_log Notice "tickets.tcl(252): mode $mode next_mode $next_mode"
    if { $mode eq "l" } {
        if { $read_p } {
            set validated_p 1
            ns_log Notice "tickets.tcl validated for l"
        } else {
            set mode ""
            set next_mode ""
        }
    }
    ns_log Notice "tickets.tcl(262): mode $mode next_mode $next_mode"
    if { $mode eq "v" } {
        if { $read_p } {
            ...
            if { $page_id_from_url ne "" } {
                ...
                set mode "l"
                ns_log Notice "tickets.tcl(405): mode = $mode ie. list of pages, index"
            }
        } else {
            set mode ""
            set next_mode ""
...
#                qw_page_delete $page_id $page_template_id_from_url $instance_id $user_id
        ...
        ns_log Notice "tickets.tcl(358): mode $mode"
        if { $mode eq "a" } {
            # change active revision of page_template_id_from_url to page_id
            if { $write_p } {
                if { [qw_change_page_id_for_url $page_id $url $instance_id] } {
                    set mode $next_mode
        ...
        ns_log Notice "tickets.tcl(344): mode $mode"
        if { $mode eq "t" } {
            #  toggle trash
            ns_log Notice "tickets.tcl mode = trash"
...
#            set template_id \[lindex $page_id_stats 5\]
            ...
            if { $write_p || $page_user_id eq $user_id } {
                ...
                ns_log Notice "tickets.tcl(419): qw_page_trash page_id $page_id trash_p $trash templat_id $page_template_id"
            ...
            set next_mode ""
            # update the page_id
            set page_id_from_url [qw_page_id_from_url $url $instance_id]
            if { $page_id_from_url ne "" && $mode eq "" } {
                set mode "v"
            }
        }
        ns_log Notice "tickets.tcl(374): mode $mode"
        if { $mode eq "w" } {
            if { $write_p } {
                ns_log Notice "tickets.tcl permission to write the write.."
                set page_contents_quoted $page_contents
                set page_contents [ad_unquotehtml $page_contents]
                set allow_adp_tcl_p [parameter::get -instance_id $instance_id -parameter AllowADPTCL -default 0]
                set flagged_list [list ]
                
                if { $allow_adp_tcl_p } {
                    ns_log Notice "tickets.tcl(311): adp tags allowed. Fine grain filtering.."
                    # filter page_contents for allowed and banned procs in adp tags
                    set banned_proc_list [split [parameter::get -instance_id $instance_id -parameter BannedProc]]
                    set allowed_proc_list [split [parameter::get -instance_id $instance_id -parameter AllowedProc]]
                        ...
                        foreach code_segment $code_segments_list  {
                            ...
                            foreach executable_fragment $executable_fragment_list {
                                ...
                                if { $executable eq "" } {
                                    ...
                                    # ns_log Notice "tickets.tcl(395): executable is empty. Screening incomplete?"
                                } else {
            ...
            util_user_message -message [lindex $user_message_list end]
                    ...
                    ns_log Notice "tickets.tcl(358): filtering out adp tags"
                    # ns_log Notice "tickets.tcl(359): range page_contents 0 120: '[string range ${page_contents} 0 120]'"
                    set page_contents_list [qf_remove_tag_contents "<%" "%>" $page_contents]
                    set page_contents_filtered [join $page_contents_list ""]
                    # ns_log Notice "tickets.tcl(427): range page_contents_filtered 0 120: '[string range ${page_contents_filtered} 0 120]'"
                }
                # use $page_contents_filtered, was $page_contents
                set page_contents [ad_quotehtml $page_contents_filtered]
                
                if { [llength $flagged_list ] > 0 } {
                    ns_log Notice "tickets.tcl(369): content flagged, changing to edit mode."
                    set mode e
                } else {
                    # write the data
                    ...
                    if { $page_id eq "" } {
                        # create page
                        set page_id [qw_page_create $url $page_name $page_title $page_contents_filtered $keywords $description $page_comments $page_template_id $page_flags $instance_id $user_id]
                        if { $page_id == 0 } {
                            ns_log Warning "q-wiki/tickets.tcl page write error for url '${url}'"
                            lappend user_messag_list "There was an error creating the wiki page at '${url}'."
                        }
                    } else {
                        # write page
                        set page_id [qw_page_write $page_name $page_title $page_contents_filtered $keywords $description $page_comments $page_id $page_template_id $page_flags $instance_id $user_id]
                        if { $page_id eq "" } {
                            ns_log Warning "q-wiki/tickets.tcl page write error for url '${url}'"
                    ...
                    if { $url ne $page_name } {
                        ...
                        if { [qw_page_rename $url $page_name $instance_id ] } {
                    ...
                    # switch modes..
                    ns_log Notice "tickets.tcl(396): activating next mode $next_mode"
                    set mode $next_mode
                }
            } else {
		...
		util_user_message -message [lindex $user_message_list end]
                ns_log Notice "tickets.tcl(402) User attempting to write content without permission."
                if { $read_p } {
                    set mode "v"
                } else {
                    set mode ""
...
switch -exact -- $mode {
    l {
        ...
        if { $read_p } {
            ...
            set page_ids_list [qw_pages $instance_id]
            ...
            ns_log Warning "tickets.tcl:(465) user did not get expected 404 error when not able to read page."
        }
    }
    r {
        ...
        if { $write_p } {
            ns_log Notice "tickets.tcl mode = $mode ie. revisions"
            # build menu options
            lappend menu_list [list #q-wiki.edit# "${url}?mode=e" ]
            ...
            set page_ids_list [qw_pages $instance_id $user_id $template_id]

            set pages_stats_lists [list ]
            # we get the entire data set, 1 row(list) per revision as table pages_stats_lists
            # url is same for each
            set page_id_active [qw_page_id_from_url $url $instance_id]
    ...
    e {
        ...
        if { $write_p } {
            ...
            ns_log Notice "tickets.tcl mode = edit"
            ...
            ns_log Notice "tickets.tcl(636): conn_package_url $conn_package_url post_url $post_url"
            if { $page_id_from_url ne "" && [llength $user_message_list ] == 0 } {

                # get page info
                set page_list [qw_page_read $page_id_from_url $instance_id $user_id ]
    ...
    v {
        ...
        if { $read_p } {
            ...
            if { $redirect_before_v_p } {
                ns_log Notice "tickets.tcl(835): redirecting to url $url for clean url view"
                ad_returnredirect $url
                ad_script_abort
            }
            ns_log Notice "tickets.tcl(667): mode = $mode ie. view"
            ...
            if { $page_id eq "" } {
                ...
                set page_list [qw_page_read $page_id $instance_id $user_id ]
            }
            ...
            ns_log Warning "tickets.tcl:(619) user did not get expected 404 error when not able to read page."
        }
    }
    w {
        #  save.....  (write) page_id 
